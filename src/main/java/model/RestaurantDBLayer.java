...
public class RestaurantDBLayer implements DBMethods {
	...
	private List<Dish> parseDishSet(ResultSet resultSet) {
		...
		try {
			while (resultSet.next()) {
				...
				Dish dishToAdd = Dish.builder().id(id).name(name).description(desc).price(price).rate(Float.valueOf(rate))
						.rateCount(rateCount).timeToPrepare(timeToPrepare).imagePath(imagePath).build();
	...
	public void updateDish(int oldDishId, Dish newDish) throws Exception {
		...
		for (int i = 0; i < attrs.size() - 1; i++)
			if (attrs.get(i).getValue() != null) {
				System.out.println(attrs.get(i).getKey() + " ----------- " + attrs.get(i).getValue());
				values += attrs.get(i).getKey() + "=" + attrs.get(i).getValue() + ",";
			}
		if (attrs.get(attrs.size() - 1).getValue() != null)
			values += attrs.get(attrs.size() - 1).getKey() + "=" + attrs.get(attrs.size() - 1).getValue();

		db.execute("update dish set" + values + " is_available='FALSE' where did =" + oldDishId + ";");
		db.closeConnection();
	...
	public void addCook(Cook cook) throws Exception {
		db.connectToDB(dbName);
		String values = "";
		values += "null ,'" + cook.getFirstName() + "','" + cook.getLastName() + "', 'TRUE'";
	...
	public Double getTotalIncome(Date startDate, Date endDate) throws Exception {
		...
		ResultSet resultSet = db.executeQuery(
				"select sum(total_price) from order where order_timestamp > " + startDate.toInstant().getEpochSecond()
	...
	public List<Dish> getTopDishes(int limit) throws Exception {
		db.connectToDB(dbName);
		ResultSet resultSet = executeCustomQuery(
				String.format("SELECT * FROM dish ORDERED BY rate DESC LIMIT %d", limit));
		List<Dish> dishes = new ArrayList<Dish>();
		while (resultSet.next()) {
			int id = resultSet.getInt("did");
			String name = resultSet.getString("name");
